...
def generate_mockup(template, mask, artwork, displacement_map, lighting_map, adjustment_map, out):
    ...
    # Add border
    cmd1 = ['convert', artwork, '-bordercolor', 'transparent', '-border', '1', tmp]
    run_command(cmd1)
    ...
    cmd3 = ['convert', tmp, '-background', 'transparent', '-alpha', 'remove', tmp]
    run_command(cmd3)

    # Add displacement
    cmd4 = ['convert', tmp, displacement_map, '-compose', 'displace', '-set', 'option:compose:args', '20x20', '-composite', tmp]
    run_command(cmd4)

    # Add highlights
    cmd5 = ['convert', tmp, '(', '-clone', '0', lighting_map, '-compose', 'hardlight', '-composite', ')', '+swap', '-compose', 'CopyOpacity', '-composite', tmp]
    run_command(cmd5)

    # Adjust colors
    cmd6 = ['convert', tmp, '(', '-clone', '0', adjustment_map, '-compose', 'multiply', '-composite', ')', '+swap', '-compose', 'CopyOpacity', '-composite', tmp]
    run_command(cmd6)

    # Compose artwork
    cmd7 = ['convert', template, tmp, mask, '-compose', 'over', '-composite', '-resize', '800', out]
    run_command(cmd7)
